...
if ([string]::IsNullOrWhiteSpace($OutputDirectory)) {
    ...
    $OutputDirectory = Join-Path $repoRoot "tmp/automation-captures/winmux-tab-switch-$timestamp"
}
...
try {
    ...
    $tabIds = @($thread.tabs | ForEach-Object { $_.id })
    foreach ($tabId in $tabIds) {
        $null = Invoke-AutomationPost "/action" @{ action = "selectTab"; tabId = $tabId }
        Wait-ForTerminalReady -TabId $tabId | Out-Null
        $null = Invoke-AutomationPost "/action" @{ action = "input"; value = "echo tab $tabId`r" }
        Pause-Step 600
    }
    ...
    $sequence = @(
        $tabIds[0], $tabIds[1], $tabIds[2],
        $tabIds[1], $tabIds[0], $tabIds[2],
        $tabIds[0], $tabIds[2], $tabIds[1],
        $tabIds[2], $tabIds[1], $tabIds[0]
    )

    foreach ($tabId in $sequence) {
        $null = Invoke-AutomationPost "/action" @{ action = "selectTab"; tabId = $tabId }
        Pause-Step 950
    }
...
if ($null -eq $recordingStop -or -not $recordingStop.ok) {
    throw "The tab-switch recording did not stop cleanly."
}
